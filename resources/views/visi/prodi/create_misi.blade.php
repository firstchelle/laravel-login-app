        ...
        <div class="max-w-4xl mx-auto sm: px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    {{-- Header --}}
                    <div class="mb-6">
                        <h2 class="text-2xl font-bold text-gray-800">Tambah Misi Program Studi</h2>
                        <p class="text-gray-600 mt-1">Isi formulir di bawah untuk menambahkan misi program studi</p>
                    </div>

                    {{-- Form --}}
                    <form action="{{ route('visiprodi.store_misi') }}" method="POST" enctype="multipart/form-data"
                        class="space-y-6">
                        @csrf

                        {{-- Pilih Visi dengan Select2 --}}
                        <div class="flex flex-col gap-2">
                            <label for="visi_prodi_id" class="font-semibold text-gray-700">
                                Pilih Visi <span class="text-red-500">*</span>
                            </label>
                            <select name="visi_prodi_id" id="visi_prodi_id"
                                class="border border-gray-300 rounded-lg p-3 w-full" required>
                                <option value="">-- Pilih Visi --</option>
                                @foreach ($visi as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('visi_prodi_id') == $item->id ? 'selected' : '' }}>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea name="misi" id="misi" rows="6"
                                ...
                                placeholder="Masukkan misi program studi..." required>{{ old('misi') }}</textarea>
                            @error('misi')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                        </div>
