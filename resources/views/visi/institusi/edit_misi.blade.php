        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    {{-- Header --}}
                    <div class="mb-6">
                        <h2 class="text-2xl font-bold text-gray-800">Edit Misi Institusi</h2>
                        <p class="text-gray-600 mt-1">Perbarui informasi misi institusi</p>
                    </div>

                    {{-- Form --}}
                    <form action="{{ route('visiinstitusi.update_misi', $item->id) }}" method="POST"
                            ...
                            <select name="visi_institusi_id" id="visi_institusi_id"
                                ...
                                @foreach ($visi as $visiItem)
                                    <option value="{{ $visiItem->id }}"
                                        {{ old('visi_institusi_id', $item->visi_institusi_id) == $visiItem->id ? 'selected' : '' }}>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea name="misi" id="misi" rows="6"
                                ...
                                placeholder="Masukkan misi institusi..." required>{{ old('misi', $item->misi) }}</textarea>
                            @error('misi')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                        </div>
