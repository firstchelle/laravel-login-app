        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Tanggal Dibuat</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Dokumen</th>
                                ...
                                @forelse ($capaian as $item)
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            <div class="font-semibold text-gray-800">{{ $item->capaian_profil_lulusan }}</div>
                                        </td>

                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            {{ \Illuminate\Support\Str::limit($item->deskripsi_capaian_profil_lulusan, 60) }}
                                        </td>

                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            @if ($item->profilLulusan)
                                                <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800 font-semibold">
                                                    PROFIL
                                                </span>
                                                <div class="text-xs text-gray-600 mt-1">
                                                    {{ \Illuminate\Support\Str::limit($item->profilLulusan->profil_lulusan, 40) }}
                                                </div>
                                            @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex justify-center gap-3">
                                                    ...
                                                    {{-- Delete --}}
                                                    <form action="{{ route('capaian.destroy', $item->id) }}" method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                    ...
                    {{-- Pagination --}}
                    @if (method_exists($capaian, 'hasPages') && $capaian->hasPages())
                        <div class="mt-6">
                            {{ $capaian->links() }}
                        </div>
                    @endif
